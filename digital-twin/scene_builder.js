...
function buildHullGeometries() {
  ...
  // マージンは固定値ではなくWAVE_AMPLITUDE_SUMに比例させる。固定0.16は多層船体導入時（ec6d4c9）に
  // 当時のWAVE_AMPLITUDE_SUM=0.87（振幅0.35/0.22/0.3の合計）を前提にgl.readPixels()で実測校正した値だったが、
  // 優先度4で波の可視性のため振幅を約1.7倍（0.35/0.22/0.3 → 0.55/0.4/0.5、WAVE_AMPLITUDE_SUM=1.45）に
  // 引き上げた際にこのマージンを再校正しなかったため、水面がチャインより高く持ち上がり防汚色帯が
  // 通常視点でほぼ常時水没する回帰が発生した（実測: 帯位置のピクセルが防汚色RGB(179,80,46)ではなく
  // 海面色RGB(102,192,207)を返す）。crestの0..1正規化をWAVE_AMPLITUDE_SUM基準にしたのと同じ考え方で
  // 比率を保つことで、今後振幅を再調整してもマージンが自動的に追従し、この種の「片方を直して
  // 片方を壊す」回帰を防ぐ。
  const BOOT_TOPPING_MARGIN_RATIO = 0.16 / 0.87; // 導入時の校正値(0.16)を当時の振幅合計(0.87)で正規化した比率
  const waterlineY = -SHIP_DECK_HEIGHT / s + BOOT_TOPPING_MARGIN_RATIO * WAVE_AMPLITUDE_SUM;
