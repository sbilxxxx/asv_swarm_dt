...
| A-7 | **`coord.js` のモジュールグローバルorigin**。別originのシナリオで`createSceneGeometry()`を呼ぶと既存ワールドのGNSSが化ける。「グローバル変数非依存・並列実行を妨げない」という自らの設計原則に違反 | 実測: (35.45,139.75)→(34.00,133.50)に変化 | **対応済** — `core/coord.js`のモジュールグローバル`origin`/`setOrigin()`/`getOrigin()`を廃止し、`createProjection(originLatLon)`がシーンごとに独立した`{origin, latLonToLocal, localToLatLon}`を返す形にした（バックワード互換シムなし）。`scene_format.js`の`createSceneGeometry()`がこのprojectionを`SceneGeometry.projection`として保持し、`GnssSensor`は`world.scene.projection`経由で変換する（`core/sim/sensors/gnss.js`）。呼び出し側（`digital-twin/main.js`・`swarm-sim/main.js`・`scripts/headless_run.js`・`tests/core_smoke.test.js`）はすべて`scene.projection.latLonToLocal/localToLatLon`を使うよう更新。`tests/core_smoke.test.js`に`testCoordProjectionIsolatedPerScene`を追加し、レビューと同じ2origin構成（35.45,139.75→34.0,133.5）で1つ目のWorldのGNSSが変化しないこと・2つ目のWorldは自身のoriginを正しく反映することを回帰確認（`node tests/core_smoke.test.js`でPASS） |
...
| A-9 | **boundsの定義と実データの矛盾**。`scene_format.js` はboundsを「coastline＋運用エリア」と定義し警告まで書いているが、シナリオJSONに `spawnsAreaLatLon` が無く、全スポーン（y≈-100〜-300m）がbounds（minY≈+250m）の外。さらに `manual_coastline.js` は `spawnsAreaLatLon` を`createSceneGeometry`に渡し忘れ | 2D表示は横長スケールのため偶然救われているだけ | **対応済** — `core/scenarios/tokyo_bay_minimal.json`に`spawnsAreaLatLon`（spawns3隻・protectedAssetを全て包含する矩形、lat 35.443〜35.452 / lon 139.744〜139.758）を追加し、`manual_coastline.js`が`spawnsAreaLatLon`（と`landmarkSet`）を確実に`createSceneGeometry()`へ渡すよう修正（以前は渡し忘れていた）。digital-twin/swarm-sim双方をheadless Chromeでスクリーンショット確認し、コンソールに境界警告が出ないこと・2D/3Dとも船体とprotectedAssetが画角に収まっていることを確認 |
...
| # | 指摘 | 状態 |
|---|------|------|
| B-1 | **アダプターレジストリ未使用**。`staticGeometryAdapters` を参照するコードはゼロ。両main.jsは `createSceneGeometry(scenario)` 直呼び。シナリオJSONにadapter指定フィールドも無く、選択メカニズム自体が存在しない。「差し込み口はすでに設計済み」は現状成立しない（アダプターを追加しても誰も呼ばない） | **対応済** — `core/data/adapters/index.js`に`loadSceneFromScenario(scenario)`を追加。`scenario.adapter`（未指定・不明なら明示的にthrow）で`staticGeometryAdapters`からアダプターを選び、`SceneGeometry`を返す唯一のロード経路にした。シナリオJSONに`"adapter": "manual_coastline"`を追加し、`digital-twin/main.js`・`swarm-sim/main.js`・`scripts/headless_run.js`をすべて`createSceneGeometry(scenario)`直呼びから`loadSceneFromScenario(scenario)`経由に置き換えた |
| B-2 | **`DynamicObservationAdapter.stream()`（AIS/ドローン）の消費者ゼロ**。streamをWorldに流し込む取込経路が無い | **未対応** |
| B-3 | **`environment.sample()` 呼び出しゼロ**。`world.js` で保持するだけで `asv.js` の運動学は環境力を参照しない。**波サロゲートモデルへの「差し替え口」は、差し替えても何も変わらない口** | **対応済** — `AsvPlatform.step()`に`environment`・`t`（シム時刻）引数を追加し、`environment.sample(x, y, t)`が返す`currentX`/`currentY`をdt倍して位置へ加算するよう配線（`core/sim/platforms/asv.js`）。呼び出し元は`EnvApi.step()`の1箇所のみで、`this.world.environment`・`this.world.clock`を渡すよう更新（`core/env/env_api.js`）。`CalmSeaEnvironment`は引き続き`currentX=currentY=0`を返すため既定シナリオの挙動・見た目は変化しない。`tests/core_smoke.test.js`に`testEnvironmentSampleIsWiredIntoKinematics`を追加し、non-zeroな`currentX`を返すテスト用environmentに差し替えると同じstep数・同じactionsでも位置が測定可能な分だけずれる（CalmSeaとの差分が`steps*dt*driftMps`に一致）ことを回帰確認 |
| B-4 | **`obstacles` はスキーマのみ**。型定義以外に参照ゼロ。3D/2Dとも描画されず、衝突判定も無く、シナリオJSONにデータも無い | **対応済（レビュー提示の「当面スキーマから外す」案を採用）** — `scene_format.js`の`SceneGeometry` JSDocおよび`createSceneGeometry()`から`obstacles`フィールドを削除し、ファイルヘッダに「描画・衝突判定を実装するまでスキーマから外した」旨のコメントを追加。`manual_coastline.js`の`obstacles`受け渡しも削除。リポジトリ全体をgrepし、`docs/`の指摘文言以外に参照が残っていないことを確認済み |
| B-5 | **AUV「登録するだけで済む」は不成立**。`EntityState` に深度(z)が無く、radar/gnss/commsも2D前提。実際はstate・センサーの改修が必要 | **対応済（ドキュメント訂正のみ）** — 誤解を招く記述の出どころだった`core/sim/platforms/index.js`の「ここに登録するだけで済む」を、「登録自体は1行だが、`EntityState`への深度(z)フィールド追加とGNSS/レーダー/通信センサーの3D対応が別途必要（現状は2D専用）」という正直な記述へ修正。`core/sim/platforms/asv.js`のヘッダコメントにも同趣旨の注記を追加。AUV自体の実装は行っていない（対象外） |
...
| B-8 | **「海域を固定しない」原則と `landmarks.js` の矛盾**。`scene_builder.js` が無条件に `buildLandmarks()`（東京タワー・レインボーブリッジ・富士山・お台場を絶対座標でハードコード）を追加。シナリオを別海域に差し替えても東京の景観が描かれる | **対応済（最小対応）** — シナリオJSONに任意の`"landmarkSet": "tokyo_bay"`フィールドを追加し、`loadSceneFromScenario()`経由で`SceneGeometry.landmarkSet`まで届くようにした。`digital-twin/scene_builder.js`は`landmarkSet`名→ビルダー関数のレジストリ（`LANDMARK_BUILDERS = { tokyo_bay: buildLandmarks }`）を持ち、既知のセットが指定されたときだけランドマークを描画する（未指定・未知の値なら描画せず`console.warn`のみ）。デフォルトシナリオは`"tokyo_bay"`を維持しているため、既存デモの見た目は変化しない。`landmarks.js`自体は東京湾セット専用のまま手を加えていない |
...
| 3 | レーダー左右反転修正と、GNSS headingの北基準CW・0〜360°正規化表示（`gnss.js`は数学規約の生値で360°超えも表示される） | ~30分 | **対応済** — GNSS表示側も対応。`digital-twin/hud.js`の`renderGnssPanel()`に`toCompassHeadingDeg(mathDeg)`（`compassDeg = (90 - mathDeg + 360) % 360`）を追加し、表示直前にのみ適用（`gnss.js`のセンサー出力・数学規約は不変のまま、steering等の依存箇所に影響なし）。headless Chromeで実測: 数学規約heading≈70°/72°の艇でGNSS表示が「heading 20°」→「heading 18°」と北基準・時計回りで正しく変化することを確認（値が0〜360の範囲に収まり負値・360超えは出ない） |
...
| 6 | coord.jsの脱シングルトン化。originをSceneGeometryに持たせる | ~1時間 | **対応済** — 詳細はA-7/E-6行を参照。`createOriginProjection(originLatLon)`が返す変換関数のペアを`SceneGeometry.projection`として持たせ、モジュールグローバルoriginを廃止した |
...
| 8 | レジストリの配線。シナリオJSONに `"adapter": "manual_coastline"` を持たせ、両mainが registry 経由でロード。`spawnsAreaLatLon` も通す。`obstacles` は描画するか当面スキーマから外す | ~1時間 | **対応済** — B-1行の通り`loadSceneFromScenario()`を配線し、`spawnsAreaLatLon`もA-9対応で通した。`obstacles`はB-4行の通りスキーマから除去（描画未実装のため） |
| 9 | `environment.sample()` を `asv.js` のstepで参照し、波モデル差し込み口を「効く」状態にする。3Dカメラ演出の改善も併せて | ~2-3時間 | **対応済（配線のみ。3Dカメラ演出は本タスクの対象外）** — 詳細はB-3行を参照。`environment.sample()`が実際に運動学へ効くようになったが、`CalmSeaEnvironment`は引き続きほぼ無効果を返すため見た目・既存デモへの影響は無い |
