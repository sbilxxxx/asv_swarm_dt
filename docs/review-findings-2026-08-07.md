...
| 6 | coord.jsの脱シングルトン化。originをSceneGeometryに持たせる | ~1時間 | **対応済** — 詳細はA-7/E-6行を参照。`createProjection(originLatLon)`が返す変換関数のペアを`SceneGeometry.projection`として持たせ、モジュールグローバルoriginを廃止した |
